...
class AttributeContext:
    """Entity that manages Chef node attribute values throughout execution.

    This tracks attribute assignments as recipes execute,
    maintaining a nested dictionary of attribute values.
    """

    def __init__(self):
        self._attributes: dict[str, Any] = {}

    def set_attribute(self, attribute_path_str: str, value: Any) -> None:
        """Set an attribute value from path string."""
        attr_path = AttributePath(attribute_path_str)
        if not attr_path.is_valid():
            return

        self._set_nested(attr_path.keys, value)
    ...
    def get_attribute(self, keys: list[str]) -> Any | None:
        """Get attribute value from key path."""
        current = self._attributes
        for key in keys:
    ...
    def load_defaults(self, defaults: dict[str, Any]) -> None:
        """Load default attributes from nested dict."""
        self._merge_defaults("", defaults)

    def _merge_defaults(self, prefix: str, value: Any) -> None:
        """Recursively merge default attributes."""
        ...
        for key, nested_value in value.items():
            ...
            if isinstance(nested_value, dict):
                self.set_attribute(current_path, nested_value)
                self._merge_defaults(current_path, nested_value)
            else:
                self.set_attribute(current_path, nested_value)
