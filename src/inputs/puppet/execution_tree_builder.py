...
if TYPE_CHECKING:
    ...
    from .recursive_analyzer import RecursiveManifestAnalyzer
...
class PuppetExecutionTreeBuilder:
    ...
    def __init__(
        ...
        recursive_analyzer: RecursiveManifestAnalyzer | None = None,
    ):
    ...
    def _expand_class(self, class_name: str) -> ExecutionTreeNode:
        ...
        if manifest is None and self.recursive_analyzer:
            manifest = self.recursive_analyzer.get_or_analyze_class(class_name)
            if manifest:
                self._manifest_map[class_name] = manifest
