...
class TestValidationIntegration:
    ...
    def test_new_validation_path_with_feature_flag(self, mock_state, monkeypatch):
        ...
        assert hasattr(agent, "validation_service")
        assert len(agent.validators) == 2
        assert isinstance(agent.validators[0], AnsibleLintValidator)
        assert isinstance(agent.validators[1], RoleStructureValidator)
        assert isinstance(agent.validation_service, ValidationService)
    ...
    def test_backward_compatibility_of_validator_methods(self, tmp_path):
        ...
        # Mock the new validators
        with patch.object(agent.validators[0], "validate") as mock_lint_validate:
            mock_lint_validate.return_value = Mock(
                success=True,
                message=ANSIBLE_LINT_TOOL_SUCCESS_MESSAGE,
                validator_name="ansible-lint",
            )

            # Call old-style method (should delegate to new validator)
            success, message = agent._validate_ansible_lint(state)

            # Verify backward compatibility
            assert success is True
            assert message == ANSIBLE_LINT_TOOL_SUCCESS_MESSAGE
            mock_lint_validate.assert_called_once()
